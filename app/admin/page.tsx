...
export default function AdminDashboard() {
  ...
  // 🔑 Хөгжүүлэлтийн явцад гацалт үүсгэхгүйн тулд хамгаалалтыг хамгийн найдвартай хэлбэрээр тавьлаа.
  useEffect(() => {
    const token =
      typeof window !== "undefined" ? localStorage.getItem("token") : null;

    if (!token) {
      // Хэрэв токен байхгүй бол логин руу шилжүүлнэ
      router.push("/login");
    } else {
      fetchMenuData();
    }
    ...
    <div className="flex min-h-screen bg-[#F8F9FA] text-[#333333] font-sans">
            ...
            .map((cat) => (
              <div key={cat.id} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {/* Шинээр хоол нэмэх урамшуулалт хоосон карт */}
                  <div
      ...
      <AddDishModal
        ...
        categories={categories.map((cat: any) => ({
          id: cat.id,
          name: cat.categoryName, // name-ийг categoryName-ээр сольж байна
        }))}
        foodToEdit={editingFood}
        onSuccess={() => {
          fetchMenuData(); // Датагаа дахин татаж шинэчилнэ
        }}
        onDelete={async (id: string) => {
          const token = localStorage.getItem("token"); // 🔑 Токеноо авна

          const res = await fetch(`/api/foods/${id}`, {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`, // 👈 ЭНЭ ЗААВАЛ БАЙХ ЁСТОЙ
            },
          });

          if (res.ok) {
            fetchMenuData(); // Жагсаалтыг шинэчилнэ
